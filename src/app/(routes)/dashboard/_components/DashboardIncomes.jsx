...
export default function DashboardIncomes({
  ...
  const totalIncomeAmount =
    incomeData.reduce((a, v) => (a = a + v.amount), 0).toFixed(2) || 0;
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end">
        <div className="flex flex-col gap-2 mb-5">
          <h3 className="font-semibold">
            ...
            <span className="font-normal">{totalIncomeQuantity}</span>
          </h3>
          <h3 className="font-semibold">
            Total Income Amount:{" "}
          ...
          <div className=" overflow-y-scroll max-h-[250px] pt-5">
            <ul>
              {filteredIncomes.length === 0 && (
                <li>No incomes found during this time period.</li>
              )}
              {filteredIncomes.map((income, idx) => (
                <li
                  key={income.id || idx}
                  className="flex gap-4 items-center border border-gray-300 rounded-md py-2 odd:bg-gray-200"
                >
                  ...
                  <span>
                    {new Date(
                      income.income_created_timestamp
                    ).toLocaleDateString()}
                  </span>
                </li>
              ))}
