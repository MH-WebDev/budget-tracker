...
export default function DashboardBudgets({ budgetData, expenseData, userData, userCurrencySymbol}) {
    ...
    const activeBudgetIds = new Set(
      expenseData
        .filter(exp => filteredDateStrings.includes(new Date(exp.updated_at).toISOString().slice(0, 10)))
          ...
          <div className="flex flex-row gap-5 justify-center w-full">
              <div className="w-full border rounded-md p-5">
                  ...
                  <AreaChart
                    ...
                    chartWidth={"100%"}
                    chartHeight={"450px"}
                    chartTitle={daysFilter === 0 ? "Budgets and their expenses since account creation" : `Budgets and their relevant expenses over the past ${daysFilter} days`}
